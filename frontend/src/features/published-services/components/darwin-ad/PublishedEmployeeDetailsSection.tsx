/**
 * Published Darwin AD — POST /getemployeedetails section.
 */
...
import { usePublishedGetEmployeeDetails } from '../../hooks/usePublishedDarwinAd';
import type { PublishedSectionProps } from '../publishedSectionProps';

export const PublishedEmployeeDetailsSection = ({ notify, apiKey }: PublishedSectionProps) => {
  const [employeeIds, setEmployeeIds] = useState('');
  const mutation = usePublishedGetEmployeeDetails();
  ...
  const handleRun = async () => {
    ...
    try {
      ...
      notify({
        severity: 'success',
        summary: 'Success',
        detail: 'Employee details fetched',
        life: 3000,
      });
    } catch (error) {
      notify({
  ...
  return (
    <CollapsibleSection
      title="POST /getemployeedetails"
      subTitle="Employee details by comma-separated ids or company emails (form: EmployeeIDs)"
    >
      <div className="grid">
        <div className="col-12 md:col-8">
          <label htmlFor="ed-ids" className="block font-medium mb-2">
            EmployeeIDs
          </label>
          <InputText
            id="ed-ids"
