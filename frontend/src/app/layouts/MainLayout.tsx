...
export const MainLayout = () => {
  ...
  const visibleItems = filterNavItemsByRbac(navItems, menuKeys, rbacLoaded);
  ...
  const sections = groupNavItemsBySection(visibleItems);
  ...
  return (
    <div
      className="flex"
      style={{ height: '100vh', overflow: 'hidden', background: 'var(--color-surface-ground)' }}
    >
      {/* ─── Sidebar ─── */}
      <aside
        className="em-sidebar flex-shrink-0 flex flex-column transition-all transition-duration-200"
        style={{ width: sidebarWidth, height: '100%', overflow: 'hidden' }}
      ...
      {/* ─── Main Content Area ─── */}
      <div className="flex-1 flex flex-column" style={{ minWidth: 0, minHeight: 0 }}>
        {/* Top Bar */}
        <header
