...
async def import_employees(
    ...
    # Attached to the request session: get_db_session still owns the commit,
    # this only provides per-employee savepoints.
    ...
    for emp_data in employee_data_list:
        ...
        try:
            # Each employee is written inside its own SAVEPOINT. Without this, a
            # database error on one record leaves the session unusable and every
            # later record fails too, so the per-employee results below would be
            # misleading.
            async with uow.savepoint():
                ...
                details_fields = _extract_details_fields(
                    emp_data, user_model.id, current_user.username
                )
            ...
            # Data-level problems are expected: report the row and keep going.
            failed_count += 1
            logger.warning("Import failed for employee %s: %s", emp_id, exc)
            results.append(ImportResult(
                employee_id=emp_id,
                status="failed",
                message=str(exc)[:200],
            ...
            # Counters are only incremented once the row has actually committed
            # to its savepoint, so they can never over-report success.
...
def _extract_details_fields(
    emp_data: dict[str, Any], user_id: int, created_by: str
) -> dict[str, Any]:
